...
import { brToNumber, getCertificateStatus } from '../utils/helpers';
...
export const EquipmentScreen: React.FC<EquipmentScreenProps> = ({
    ...
    const certificateStatusFilter = query.get('certificate_status');
    ...
    const allAssets = useMemo((): DisplayAsset[] => {
        ...
        let onlyVessels = false;
        if (certificateStatusFilter) {
            const statusText = certificateStatusFilter === 'expired' 
                ? 'VENCIDO' 
                : certificateStatusFilter === 'expiring_soon' 
                ? 'VENCE EM BREVE'
                : null;
            if (statusText) {
                filteredVessels = vessels.filter(v => getCertificateStatus(v.expiryDate).text === statusText);
                onlyVessels = true;
            }
        }

        const complex: DisplayAsset[] = filteredVessels.map(v => ({
            id: `vessel-${v.id}`,
            name: v.name,
        ...
        const all = onlyVessels ? [...complex] : [...simple, ...complex];

        return all.filter(a => a.name.toLowerCase().includes(assetSearch.toLowerCase()));
    }, [simpleAssets, vessels, assetSearch, certificateStatusFilter]);
    ...
    return (
        <main className="max-w-8xl mx-auto p-4 md:p-8">
            ...
            {certificateStatusFilter && (
                <div className="mb-6 bg-secondary p-4 rounded-lg flex items-center justify-between animate-fade-in">
                    <div>
                        <p className="font-semibold">Filtro Ativo: Certificados</p>
                        <p className="text-sm text-muted-foreground">
                            Exibindo embarcações com certificados 
                            {certificateStatusFilter === 'expired' ? ' vencidos.' : ' vencendo em breve.'}
                        </p>
                    </div>
                    <Button variant="ghost" onClick={() => navigate('/registration-hub')}>
