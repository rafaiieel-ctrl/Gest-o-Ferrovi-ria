...
const App: React.FC = () => {
    ...
    // FIX: The error "Expected 1 arguments, but got 0" for useLocalStorage with a no-argument generator function is likely a linter or TypeScript inference issue with the arrow function wrapper `() => generatePlanningGoals()`. Passing the function reference `generatePlanningGoals` directly is a cleaner way for lazy initialization and resolves this error.
    ...
    const addTank = useCallback((template?: Partial<Tank>) => {
        setTanks(prev => [...prev, {
            ...
            prod: 'anidro',
            ident: '', tanque: '', cliente: '', tdesc: '', ldesc: '', vamb: '', rho: '',
            Ta: '', Tt: '', lacres: [],
    ...
    const concludeOperation = useCallback(() => {
        ...
        if (scheduleToUpdate && concludedOpType) {
            if (concludedOpType === 'loading') {
                ...
                if (scheduleToUpdate.planningGoalId) {
                    setPlanningGoals(prevGoals => prevGoals.map(goal => {
                        if (goal.id === scheduleToUpdate.planningGoalId) {
                            const currentRealized = goal.realizedVolume || 0;
                            return { ...goal, realizedVolume: currentRealized + totalLoadedVolume };
                        }
                        return goal;
                    }));
                }
    ...
    }, [activeOperation, activeScheduleForOperation, activeOperationType, setSchedule, setArchivedOperations, setActiveOperation, showToast, setActiveScheduleForOperation, setActiveOperationType, triggerNotification, setPlanningGoals]);
    ...
    const handleStartFerroviarioLoading = useCallback((scheduleId: number) => {
        ...
        if (scheduleToStart.status === 'PLANEJADO' || scheduleToStart.status === 'AGUARDANDO CARREGAMENTO') {
            if (scheduleToStart.vagoes.length === 0) {
                const newVagoes = scheduleToStart.rateios.flatMap(rateio => 
                    Array.from({ length: brToNumber(rateio.qtd_vagoes) || 0 }).map(() => ({
                        ...
                        rateioId: rateio.id,
                        statusNotaRemessa: 'PENDENTE',
                        statusNotaEmitida: 'PENDENTE',
                        statusCte: 'PENDENTE',
                    }))
                );
    ...
    const unifiedSchedules = useMemo((): UnifiedSchedule[] => {
        ...
        const ferroviario: UnifiedSchedule[] = ferroviarioSchedules.map(s => ({
            ...
            onView: handleStartFerroviarioLoading,
        }));
    ...
    const VesselScreenWrapper = () => {
        const { id } = useParams();
        const vesselId = id === 'new' ? 'new' : Number(id);

        if (id !== 'new' && isNaN(vesselId as number)) {
            showToast('ID da embarcação inválido.', 'error');
            return <Navigate to="/registration-hub" replace />;
        }

        const vessel = vesselId === 'new' ? undefined : vessels.find(v => v.id === vesselId);
        
        if (vesselId !== 'new' && !vessel) {
            showToast('Embarcação não encontrada.', 'error');
            return <Navigate to="/registration-hub" replace />;
        }
    ...
    const FerroviarioLoadingScreenWrapper = () => {
        ...
        const handleUpdateSchedule = (updatedSchedule: FerroviarioSchedule) => {
            const originalSchedule = ferroviarioSchedules.find(s => s.id === updatedSchedule.id);
    
            if (originalSchedule && updatedSchedule.planningGoalId) {
                const goal = planningGoals.find(g => g.id === updatedSchedule.planningGoalId);
                const isLiquid = goal && (goal.product === 'anidro' || goal.product === 'hidratado' || goal.product === 'etanol-mix');
    
                const newlyLoadedVagoes = updatedSchedule.vagoes.filter(updatedVagao => {
                    const originalVagao = originalSchedule.vagoes.find(v => v.id === updatedVagao.id);
                    return originalVagao && originalVagao.status !== 'CARREGADO' && updatedVagao.status === 'CARREGADO';
                });
    
                if (newlyLoadedVagoes.length > 0) {
                    setPlanningGoals(prevGoals => {
                        const goal = prevGoals.find(g => g.id === updatedSchedule.planningGoalId);
                        const isLiquid = goal && (goal.product === 'anidro' || goal.product === 'hidratado' || goal.product === 'etanol-mix');
        
                        const volumeToAdd = newlyLoadedVagoes.reduce((sum, vagao) => {
                            const volumeStr = isLiquid ? vagao.volumeObservado : vagao.pesoCarregado;
                            return sum + brToNumber(volumeStr || '0');
                        }, 0);
                        
                        if (volumeToAdd <= 0) {
                            return prevGoals; // No change
                        }
        
                        return prevGoals.map(g => {
                            if (g.id === updatedSchedule.planningGoalId) {
                                const currentRealized = g.realizedVolume || 0;
                                return { ...g, realizedVolume: currentRealized + volumeToAdd };
                            }
                            return g;
                        });
                    });
            ...
            setFerroviarioSchedules(prev => prev.map(s => s.id === updatedSchedule.id ? updatedSchedule : s));
        };
        ...
        <div className="min-h-screen pb-20 md:pb-0 md:pl-20">
            ...
            <main className="transition-all duration-300">
                 <Routes>
                    ...
                    <Route path="/registration-hub" element={<EquipmentScreen
